...
if ($pass != '') {
    if ($pass == $access_key) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            (SELECT sum(Quantity) FROM omcs.dealers_nozzels_sales where NozzleNo=$NozzleNo order by id desc) as total_qty_sale, 
            (SELECT sum(Amount) FROM omcs.dealers_nozzels_sales where NozzleNo=$NozzleNo order by id desc) as total_amount_sale 
