...
function get_task_inspection_response($connect, $task_id, $dealer_id, $db)
{
   ...
   where ii.id=$task_id and ii.dealer_id=$dealer_id";
    ...
    while ($taskDetails = $result1->fetch_assoc()) {
        ...
        $output .= '<p style="background: #e3dede;padding: 5px 7px;text-align: center;">Stock Reconcilation ' . $product_name . '</p>
        ...
        $output .= '<h6 style="text-align: center;padding: 3px 11px;background: #f2f2f2;">
...
function smtp_mailer($to, $time, $dealer_name, $dealer_id, $task_id, $db)
{
    ...
    $file_name5 = 'files/Stock Reconcilation Report_' . md5(rand()) . '.pdf';
    ...
    $mail->Subject = $dealer_name . ' Stock Reconcilation Audit Report ' . $time; //Sets the Subject of the message
    $mail->Body = '<h1>Cnergyico.<h1><h3>Please Find details report of Stock Reconcilation Audit in attach PDF File.</h3>'; //An HTML or plain text message body
